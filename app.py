...
from flask import Flask, request, render_template_string, send_file import requests import csv import io from bs4 import BeautifulSoup

app = Flask(name) app.secret_key = "researchbot_secret"
...
"""MANIFEST = { "name": "Research Bot PRO", "short_name": "ResearchBot", "start_url": "/", "display": "standalone", "background_color": "#eef", "theme_color": "#ffffff" }

def ai_summary(text): sentences = text.split(".") return ". ".join(sentences[:3])

def search_web(query): url = f"https://duckduckgo.com/html/?q={query}" r = requests.get(url, headers=HEADERS) soup = BeautifulSoup(r.text, "html.parser") results = []
...
def search_youtube(query): url = f"https://www.youtube.com/results?search_query={query.replace(' ', '+')}" r = requests.get(url, headers=HEADERS) soup = BeautifulSoup(r.text, "html.parser") videos = []
...
summary_text = " ".join([r["title"] for r in results if r.get("title")])
...
@app.route("/export") def export(): output = io.StringIO() writer = csv.DictWriter(output, fieldnames=["type", "title", "link"]) writer.writeheader() writer.writerows(DATA_STORE) output.seek(0)
